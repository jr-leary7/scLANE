#' \code{marge} models for negative binomial data.
#'
#' @name marge
...
marge <- function(X_pred = NULL,
                  ...
                  minspan = NULL) {
  # check inputs
  if (any(unlist(lapply(c(X_pred, Y), is.null)))) stop("Some inputs to marge() are missing.")
  N <- length(Y)
  ...
  ncol_B <- ncol(B)
  cnames_2 <- cnames_log <- list(colnames(B_new))

  wic_mat_2 <- wic_mat_log <- matrix(NA, ncol = ncol_B , nrow = ncol_B )
  colnames(wic_mat_2) <- colnames(wic_mat_log) <- colnames(B)
  wic_mat_2 <- cbind(wic_mat_2, rep(NA, ncol_B ))
  wic_mat_log <- cbind(wic_mat_log, rep(NA, ncol_B ))
  colnames(wic_mat_2)[(ncol_B  + 1)] <- colnames(wic_mat_log)[(ncol_B  + 1)] <- "Forward pass model"
  ...
  WIC_log <- sum(apply(wic_mat_log[1:2, ], 1, min, na.rm = TRUE)) + log(N) * ncol_Bnew
  ...
  for (i in 2:(ncol_B - 1)) {
    if (i != (ncol_B - 1)) {
      ...
      wic1_log <- backward_sel_WIC(Y = Y, B_new = B_new_log)
      ...
      WIC_log <- sum(apply(wic_mat_log[1:(ncol_Bnew), ], 1, min, na.rm = TRUE)) + log(N) * ncol(B_new_log)
  ...
  B_final <- list(as.matrix(B[, colnames(B) %in% cnames_2[[which.min(WIC_vec_2)]]]),
                  as.matrix(B[, colnames(B) %in% cnames_log[[which.min(WIC_vec_log)]]]))
