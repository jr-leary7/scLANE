...
#' @param expr.mat Either a \code{SingleCellExperiment}, \code{Seurat}, or 
...
#' Defaults to NULL.
#' @param pt Either the output from \code{\link[slingshot]{SlingshotDataSet}} 
...
#' @param genes A character vector of genes to model. If not provided, defaults 
...
#' @param gee.bias.correction.method (Optional) Specify which small-sample bias 
...
#' @param gee.test A string specifying the type of test used to estimate the 
...
#' Defaults to FALSE.
#' @param id.vec If a GEE or GLMM framework is being used, a vector of subject 
#' IDs to use as input to \code{\link[geeM]{geem}} or 
#' \code{\link[glmmTMB]{glmmTMB}}. Defaults to NULL.
#' @param glmm.adaptive (Optional) Should the basis functions for the GLMM be 
#' chosen adaptively? If not, uses 4 evenly spaced knots. Defaults to TRUE.
#' @param approx.knot (Optional) Should the knot space be reduced in order to 
#' improve computation time? Defaults to TRUE.
#' @param n.potential.basis.fns (Optional) The maximum number of possible basis 
#' functions. See the parameter \code{M} in \code{\link{marge2}}. Defaults to 5.
#' @param n.cores (Optional) If running in parallel, how many cores should be 
#' used? Defaults to 4L.
#' @param verbose (Optional) A boolean indicating whether a progress bar should 
#' be printed to the console. Defaults to TRUE.
#' @param random.seed (Optional) The random seed used to initialize RNG streams 
...
#' structure.
#' }
#' @return A list of lists, where each element is a gene and each gene contains 
...
testDynamic <- function(expr.mat = NULL,
    ...
    random.seed = 312) {
    # check inputs
    if (is.null(expr.mat) || is.null(pt)) {
        stop("You forgot some inputs to testDynamic().")
    }

    # get raw counts from SingleCellExperiment or Seurat object &
    # transpose to cell x gene dense matrix
    if (is.null(genes)) {
    ...
    if (inherits(expr.mat, "SingleCellExperiment")) {
        ...
        expr.mat <- BiocGenerics::counts(expr.mat)[genes, ]
    } else if (inherits(expr.mat, c("dgCMatrix", "dgRMatrix"))) {
        expr.mat <- expr.mat[genes, ]
    } else {
        expr.mat <- expr.mat[genes, ]
    }
    expr.mat <- as.matrix(Matrix::t(expr.mat))
    if (!(inherits(expr.mat, "matrix") || inherits(expr.mat, "array"))) {
    ...
    if (!inherits(pt, "data.frame")) {
        stop("pt must be of class data.frame.")
    }

    # set pseudotime lineage column names automatically to prevent
    # user error (uses e.g., "Lineage_A", "Lineage_B", etc.)
    n_lineages <- ncol(pt)
    colnames(pt) <- paste0("Lineage_", LETTERS[seq(n_lineages)])

    # ensure subject ID vector meets criteria for GEE / GLMM fitting
    if ((is.gee || is.glmm) && is.null(id.vec)) {
        stop("You must provide a vector of IDs if you're
            using GEE / GLMM modes.")
    }
    if ((is.gee || is.glmm) && is.unsorted(id.vec)) {
        stop("Your data must be ordered by subject, please do so before running
            testDynamic() with the GEE / GLMM modes.")
    }
    cor.structure <- tolower(cor.structure)
    if (is.gee && !(cor.structure %in% c(
        "ar1", "independence",
        "exchangeable"
    ))) {
        stop("GEE models require a valid correlation structure.")
    }
    # check GEE testing method
    gee.test <- tolower(gee.test)
    if (is.gee & !gee.test %in% c("wald", "score")) {
        stop("GEE testing method must be one of score or wald.")
    }
    ...
    necessary_vars <- c(
        "expr.mat", "genes", "pt", "n.potential.basis.fns", "approx.knot",
        "is.glmm", "gee.bias.correction.method", "gee.test",
        "verbose", "n_lineages", "id.vec", "cor.structure", "is.gee",
        "gee.scale.fix", "glmm.adaptive", "size.factor.offset"
    )
    if (any(ls(envir = .GlobalEnv) %in% necessary_vars)) {
        no_export <- c(
            ls(envir = .GlobalEnv)[-which(ls(envir = .GlobalEnv) %in%
                necessary_vars)],
    ...
    package_list <- c(
        "scLANE", "MASS", "bigstatsr", "broom.mixed",
        "dplyr", "stats"
    )
    if (is.gee) {
        ...
        for (j in seq(n_lineages)) {
            ...
            if (!is.glmm) {
                marge_mod <- try(
                    {
                        marge2(
                            X_pred = pt[lineage_cells, j, drop = FALSE],
                            ...
                            sandwich.var = ifelse(is.null(gee.bias.correction.method),
                                FALSE, TRUE
                            ),
            ...
            null_mod_df <- data.frame(
                Y_null = expr.mat[lineage_cells, i],
                Intercept = 1
            )
            if (!is.null(id.vec)) {
            ...
            if (!is.null(size.factor.offset)) {
                ...
                null_mod_formula <- paste0(null_mod_formula, " +
                                        offset(log(1 / n_offset))")
            }
                ...
                null_mod <- try(
                    {
                        geeM::geem(null_mod_formula,
                            ...
                            scale.fix = FALSE,
                            sandwich = ifelse(is.null(gee.bias.correction.method),
                                FALSE, TRUE
                            )
                        )
                    },
            ...
            marge_dynamic_df <- summarizeModel(
                marge.model = marge_mod,
                pt = pt[lineage_cells, j, drop = FALSE],
                is.glmm = is.glmm
            )
            if (is.glmm) {
            ...
            lineage_list[[j]]$Test_Stat <- ifelse(is.gee,
                ifelse(gee.test == "wald", test_res$Wald_Stat,
                    test_res$Score_Stat
                ),
                test_res$LRT_Stat
            )
    ...
    test_stats <- purrr::imap(test_stats, \(x, y) {
        if (inherits(x, "simpleError")) {
            ...
            names(reformatted_results) <- paste0(
                "Lineage_",
                LETTERS[seq(n_lineages)]
            )
