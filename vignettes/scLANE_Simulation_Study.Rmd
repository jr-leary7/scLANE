...
```{r}
set.seed(312)
samp_idx <- sample(1:ncol(sim_counts), 200)
...
We can plot some of the model results using the `plotModels()` function. 

```{r}
plotModels(marge.preds = gene_stats$PGAM1$MARGE_Preds,
           null.preds = gene_stats$PGAM1$Null_Preds, 
           gene.counts = sim_counts[, "PGAM1"], 
           pt = pt_df, 
           gene = "PGAM1")
```
