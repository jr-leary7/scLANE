...
scoreTestGEE <- function(mod.1 = NULL,
    ...
    cor.structure = "ar1") {
    # check inputs
    if (is.null(mod.1) || is.null(mod.0) || is.null(alt.df) ||
        is.null(null.df) || is.null(id.vec)) {
        stop("Please provide all inputs to scoreTestGEE().")
    }
    ...
    if (!(inherits(mod.1, "geem") && inherits(mod.0, "geem"))) {
        stop("You must provide two geeM models to scoreTestGee().")
    }
    if (length(coef(mod.0)) != 1) {
        stop("Null GEE model must be intercept-only.")
    }
    if (length(coef(mod.1)) <= length(coef(mod.0))) {
        ...
        theta <- as.numeric(gsub(
            "\\)", "",
            gsub(".*\\(", "", mod.1$FunList$family)
        ))
        ...
        for (i in seq(groups)) {
            ...
            if (cor.structure == "independence") {
                ...
                R_i <- matrix(rho^abs(outer(seq(n_i), seq(n_i), "-")),
                    nrow = n_i, ncol = n_i
                )
            }
            ...
            A_i <- diag(V_mu_i)
            # same as taking the 1/2 power of A_i since A_i is diagonal
            A_i_sqrt <- sqrt(A_i)
            V_i <- A_i_sqrt %*% R_i %*% A_i_sqrt
            V_i_inv <- try(
                {
                    eigenMapMatrixInvert(V_i)
                },
                silent = TRUE
            )
            ...
            K_i <- diag(MASS::negative.binomial(
                theta = 1,
                link = "log"
            )$mu.eta(mod.0$eta)[group_idx])
            W_i <- K_i %*% V_i_inv %*% K_i
            W_list[[i]] <- W_i
            K_inv <- try(
                {
                    eigenMapMatrixInvert(K_i)
                },
                silent = TRUE
            )
