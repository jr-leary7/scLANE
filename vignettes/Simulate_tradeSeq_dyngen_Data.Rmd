...
```{r}
a <- ts_mf_list[[1]]$counts
de_genes <- ts_mf_list[[1]]$tde_overall
true_pt <- ts_mf_list[[1]]$prior_information$timecourse_continuous
```
...
Now we'll loop through each of the 10 datasets (in parallel using `foreach`) and process them in the same way. We use `tradeSeq`'s `evaluateK()` function to determine how many knots should be used in each GAM by choosing the value of $k$ with the lowest average AIC. 
...
Next we'll need to run `associationTest()` on each GAM to determine whether genes are differentially expressed. 

```{r}
ts_asso_test_list <- map2(.x = ts_res_all, 
                          .y = ts_mf_list, 
                          ~ associationTest(.x) %>% bind_cols(.y$tde_overall$differentially_expressed))
ts_asso_test_df <- purrr::reduce(ts_asso_test_list, bind_rows) %>% 
