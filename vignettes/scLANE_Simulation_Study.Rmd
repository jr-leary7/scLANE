...
We use `testDynamic()` to fit a `marge` model to each gene with a maximum of $k = 5$ breakpoints. 
...
ggplot(roc_df_ts, aes(x = 1 - specificity, y = sensitivity)) + 
  ...
  labs(x = "1 - Specificity", y = "Sensitivity", title = "tradeSeq", 
       caption = paste0("AUC-ROC = ", round(roc_auc_ts[1, 3], 3))) + 
...
caret::confusionMatrix(data = as.factor(ifelse(global_test_results$geneStatus == "Dynamic", 1, 0)), 
                       reference = as.factor(global_test_results$Gene_Dynamic), 
                       positive = "1", 
                       mode = "everything")
```
...
ggplot(roc_df, aes(x = 1 - specificity, y = sensitivity)) + 
  geom_line(color = "forestgreen", size = 1) + 
  geom_segment(aes(x = 0, y = 0, xend = 1, yend = 1), color = "firebrick", size = 1) + 
  labs(x = "1 - Specificity", y = "Sensitivity", title = "testDynamic", 
       caption = paste0("AUC-ROC = ", round(roc_auc[1, 3], 3))) + 
  theme_classic(base_size = 14)
```
...
```{r}
plotModels(marge.preds = gene_stats$TMEM55A$MARGE_Preds,
           null.preds = gene_stats$TMEM55A$Null_Preds, 
           gene.counts = sim_counts[, "TMEM55A"], 
           pt = pt_df, 
           gene = "TMEM55A", 
           plot.gam = TRUE)
```
