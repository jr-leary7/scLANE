#' Given estimates from the null model fit and the design matrix for 
...
#' @return A calculated score statistic for the null and alternative model 
#' when fitting a GLM.
...
score_fun_glm <- function(Y = NULL,
    ...
    XA = NULL) {
    # check inputs
    if (is.null(Y) || is.null(VS.est_list) || is.null(A_list) ||
        is.null(B1_list) || is.null(mu.est) || is.null(V.est) ||
        is.null(B1) || is.null(XA)) {
        stop("Some inputs to score_fun_glm() are missing.")
    }
    ...
    # This is not the model fit!! It just checks whether any issues
    # occur for a simple linear regression model.
    if (inherits(reg, "try-error") || any(is.na(reg$coefficients))) {
        ...
        XVX_22 <- try(
            {
                eigenMapMatrixInvert(inv.XVX_22)
            },
            silent = TRUE
        )
