...
waldTestGEE <- function(mod.1 = NULL,
    ...
    if (is.null(mod.1) || is.null(mod.0)) {
        stop("Please provide both models to waldTestGEE().")
    }
    ...
    if (!is.null(correction.method)) {
        if (!(correction.method %in% c("df", "kc"))) {
            stop("Unsupported bias correction method in waldTestGEE().")
        }
        if (correction.method == "kc" && is.null(id.vec)) {
            stop("The Kauermann and Carroll bias correction method requires
                a vector of subject IDs.")
        }
    }
    mod.1 <- mod.1$final_mod
    if (!(inherits(mod.1, "geem") && inherits(mod.0, "geem"))) {
        stop("You must provide two geeM models to waldTestGee().")
    }
    ...
    if (length(coef(mod.1)) <= length(coef(mod.0))) {
        ...
        # compute test statistic, optionally bias-adjust,
        # & estimate asymptotic p-value
        ...
        if (inherits(wald_test_stat, "try-error")) {
            ...
            p_value <- as.numeric(1 - stats::pchisq(wald_test_stat,
                df = p_alt - 1
            ))
