...
test_that("testDynamic() output", {
  ...
  expect_equal(sum(unname(sapply(gene_stats, function(x) x$Lineage_A$Model_Status)) == "MARGE model OK, null model OK"), 8)
  expect_equal(unique(sapply(gene_stats, function(x) length(x$Lineage_A))), 16)
})
...
test_that("getResultsDE() output", {
  ...
  expect_equal(ncol(test_results), 14)
...
test_that("testSlope() output", {
  ...
  expect_equal(sum(slope_test$Notes == "MARGE model error, null model OK", na.rm = TRUE), 2)
