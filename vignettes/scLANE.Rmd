...
```{r}
order_df <- data.frame(PT = sim_data$cell_time_normed)
cell_offset <- createCellOffset(sim_data)
top1k_hvgs <- getTopHVGs(modelGeneVar(sim_data), n = 1e3)
```

The default modeling framework is based on GLMs, which we use here. Parallel processing is turned on by default in order to speed things up.
...
```{r}
knitr::kable(scLANE_models$HIST1H4C$Lineage_A$Gene_Dynamics, 
...
```{r, fig.cap="Expression cascade heatmap"}
Heatmap(matrix = heatmap_mat, 
        ...
        width = 9, 
        height = 6, 
        column_title = "Dynamic genes across pseudotime",
        cluster_rows = FALSE,
