...
gene_stats <- testDynamic(expr = sim_counts[, samp_idx], 
                          pt = pt_df, 
                          parallel = TRUE, 
                          n.cores = 4, 
...
```{r}
library(tradeSeq)
## this actually errored out for me:
icMat <- evaluateK(counts = t(sim_counts_1[,1:100]), pseudotime=pt_df_1$PT, cellWeights = rep(1, length(pt_df_1$PT)),k = 3:10,
                   nGenes = 100, verbose = T)
trade.sce <- fitGAM(counts = t(sim_counts_1[,1:100]), pseudotime=pt_df_1$PT, cellWeights = rep(1, length(pt_df_1$PT)),
              nknots = 6, verbose = FALSE)
table(rowData(trade.sce)$tradeSeq$converged)
assoRes <- associationTest(trade.sce)
head(assoRes)
assoRes$Gene_Dynamic[assoRes$pvalue <= .05] <- 1
assoRes$Gene_Dynamic[assoRes$pvalue > .05] <- 0
trade_results_1 <- cbind(assoRes, geneStatus = rowData(panc_sim)[1:100,])
table(trade_results_1$Gene_Dynamic, trade_results_1$geneStatus)
```
