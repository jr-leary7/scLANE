...
```{r}
set.seed(312)
samp_idx <- sample(1:ncol(sim_counts), 100)
gene_stats <- scLANE::testDynamic(expr = sim_counts[, samp_idx], 
                          pt = pt_df, 
                          parallel = TRUE, 
                          n.cores = 4, 
                          n.potential.basis.fns = 5, 
                          track.time = TRUE)
gene_stats2 <- testDynamic(expr.mat = sim_counts[, samp_idx], 
                           pt = pt_df, 
                           parallel = TRUE, 
                           n.cores = 4, 
                           n.potential.basis.fns = 5, 
...
```{r}
dyn_results <- getResultsDE(gene_stats, p.adj.method = "bonferroni", fdr.cutoff = 0.01) %>% 
...
```{r}
mod_list <- getModelList(gene_stats)
slope_test <- testSlope(model.list = mod_list, 
                        pt = pt_df, 
                        p.adj.method = "bonferroni", 
                        fdr.cutoff = 0.01)
slope_results <- slope_test %>% 
                 inner_join((rowData(panc_sim) %>% 
                             as.data.frame() %>% 
                             mutate(gene = rownames(.))), by = c("Gene" = "gene"))
```
...
```{r}
library(tradeSeq)
# evaulateK() errors out 
icMat <- evaluateK(counts = t(sim_counts[, samp_idx]), 
                   pseudotime = pt_df$PT, 
                   cellWeights = rep(1, nrow(pt_df)), 
                   k = 3:10,
                   nGenes = length(samp_idx), 
                   verbose = TRUE)
trade.sce <- fitGAM(counts = t(sim_counts[, samp_idx]), 
                    pseudotime = pt_df$PT, 
                    cellWeights = rep(1, nrow(pt_df)), 
                    nknots = 6, 
                    verbose = FALSE)
ts_asso_test <- associationTest(trade.sce) %>% 
                mutate(Gene_Dynamic = case_when(pvalue < 0.01 ~ 1, TRUE ~ 0), 
                       geneStatus = rowData(panc_sim)[samp_idx, ])
```

The results of `tradeSeq()` aren't incredible. The sensitivity is very low, and 
...
```{r}
caret::confusionMatrix(data = as.factor(ifelse(dyn_results$geneStatus == "Dynamic", 1, 0)), 
                       reference = as.factor(dyn_results$Gene_Dynamic), 
                       positive = "1", 
                       mode = "everything")
```
...
```{r}
slope_res_sum <- slope_results %>% 
                 select(Gene, Gene_Dynamic, geneStatus) %>% 
                 distinct()
caret::confusionMatrix(data = as.factor(ifelse(slope_res_sum$geneStatus == "Dynamic", 1, 0)), 
                       reference = as.factor(slope_res_sum$Gene_Dynamic), 
                       positive = "1", 
                       mode = "everything")
```
