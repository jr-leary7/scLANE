...
The PCA embeddings show us a pretty simple trajectory that's fairly correlated with the first principal component, and we can also see that the trajectory is conserved cleanly across subjects. 
...
We re-run the DE tests a final time using the GLMM backend. This is the most complex & time-consuming type of model we support, and is also the trickiest to interpret. We recommend using it when you're most interested in how a trajectory differs between subjects e.g., if the subjects can be stratified by groups such as Treatment & Control and you expect the Treatment group to have a different progression through the biological process. Executing the function with the GLMM backend differs only in that we switch the `is.glmm` flag to `TRUE` and no longer need to specify a working correlation structure. **Note**: the GLMM backend is still very much under development, as we are attempting to reduce runtime and increase the odds of the underlying optimization process converging. As such, updates will be frequent and functionality / results may shift slightly. 
...
Like the GLM backend, the GLMMs use a likelihood ratio test to compare the null & alternate models (we fit the models using maximum likelihood instead of REML in order to perform this test). 
...
After generating a set of models, we can cluster the genes in a semi-supervised fashion using `clusterGenes()`. This function uses the Leiden algorithm (the default), hierarchical clustering, or *k*-means and selects the best set of clustering hyperparameters using the silhouette score. We then use the results as input to `plotClusteredGenes()`, which generates a table of fitted values per-gene, per-lineage over pseudotime along with the accompanying cluster labels. 
...
The results can then be plotted as desired using `ggplot2` or another visualization package. Our results could be interpreted as showing a cluster of non-dynamic genes (cluster 1) and a cluster of dynamic genes (cluster 2).
...
Indeed, if we look at the true frequency of dynamic genes in each group, we see that cluster 2 has a much higher proportion than cluster 1. 
