...
summary.scLANE <- function(
        test.dyn.res = NULL,
        p.adj.method = "fdr",
        fdr.cutoff = 0.01) {
    if (!inherits(test.dyn.res, "scLANE")) {
    ...
    summary_stats$n_significant_genes <- sum(adj_p_values < fdr.cutoff,
        na.rm = TRUE
    )
    ...
    summary_stats$test_type <- ifelse(test.dyn.res[[1]][[1]]$Test_Stat_Type == "LRT",
        ...
        ifelse(test.dyn.res[[1]][[1]]$Test_Stat_Type == "Wald", "Wald Test",
            "Score Test"
        )
    )
...
print.summary.scLANE <- function(x) {
    ...
    cat(
        "Mean adjusted p-value: ", format(x$mean_adj_p_value, digits = 5),
        "\n"
    )
    cat(
        "Number of significant gene-lineage tests: ", x$n_significant_genes,
        "\n"
    )
    invisible(x)
}
